"""
Module: compiler_warning_ops
Description: This module contains functions for generating compiler warnings report.
...
import pandas as pd
from emc_test_automation_api.src.utils.report_utils import (
    ReportOps,
)  # pylint: disable=E0401 # required import


class CompilerWarningReport:
    """
    Contains methods for compiler report validation, formatting of sheets and report generation.
    """

    def process_compiler_report(
        self, log_directory, report_path, props: Any
    ) -> OrderedDict:
        """
        ...
        """
        reports = ReportOps()
        self.driver_wise_count = (
            props.compiler_warning_report_props.constants.driver_wise_count
        )
        self.file_wise_calculation = (
            props.compiler_warning_report_props.constants.file_wise_calculation
        )
        self.log_directory = log_directory
        self.report_data = OrderedDict()

        self.filewise_count_header = (
            props.compiler_warning_report_props.filewise_count_header
        )
        self.driverwise_count_header = (
            props.compiler_warning_report_props.driverwise_count_header
        )

        # warnings_sheet_header = ["File_Path", "Line Number", "Warning", "Note", "Remarks"]
        self.warnings_sheet_header = (
            props.compiler_warning_report_props.warnings_sheet_header
        )
        # Define the regular expression pattern
        self.compiler_warning_pattern_start = (
            props.compiler_warning_report_props.compiler_warning_pattern_start
        )
        self.compiler_warning_pattern_end = (
            props.compiler_warning_report_props.compiler_warning_pattern_end
        )
        self.driver_filter_pattern_prefix = (
            props.compiler_warning_report_props.driver_filter_pattern_prefix
        )
        self.driver_filter_pattern_suffix = (
            props.compiler_warning_report_props.driver_filter_pattern_suffix
        )
        self.cfgstat_filter_pattern = (
            props.compiler_warning_report_props.cfgstat_filter_pattern
        )

        # naming cfg sheet from cfgstat_filter_pattern obtained from config properties yaml
        self.cfgstat_name = (
            self.cfgstat_filter_pattern.split("(")[1].split(")")[0].upper()
        )

        # Define a regex pattern to match the desired format
        self.compiler_file_pattern = (
            props.compiler_warning_report_props.compiler_file_pattern
        )
        ...
        self.driver_groups = {}
        self.df_data_dict = {}
        for _, _, files in os.walk(log_directory):
            for file in files:
        ...
        if self.log_files:
            ...
            # # Generate the report
            self.report_data = reports.generate_excel_report_data(
                report_path=report_path, df_data_dict=self.df_data_dict, props=props
            )
    ...
    def _generate_warnings_sheet_data(self, is_driver) -> bool:
        ...
        # Generate data for each driver group
        for driver, log_files in self.driver_groups.items():
            # Prepare driver-specific data
            ...
            # Extend lists with data for processing
        ...
        arguments = zip(
            [is_driver] * len(log_file_list),
            [self.cfgstat_filter_pattern] * len(log_file_list),
            driver_list,
            driver_filter_pattern_list,
            log_file_list,
        )
        ...
        for result in results:
            for key, value in result.items():
                final_result.setdefault(key, []).extend(value)

        # Update data dictionary with processed results
        self.df_data_dict.update(final_result)
        # Set status flag to indicate successful processing
    ...
    def _generate_filewise_calculation_sheet_data(self):
        """
        This method processes the sheetwise compiler driver / cfgstat data and
        generate a filewise count that is added to the dictionary of dataframes
        which is an attribute of the class that can then be used to generate
        the compiler warnings report.
        """
        ...
        for driver in self.df_data_dict:
            driver_name = driver.lower()
            self.df_data_dict[driver] = [
                pd.concat(self.df_data_dict[driver], ignore_index=True)
            ]
            # Extract filenames from filepath
            file_info = (
                self.df_data_dict[driver][0][self.warnings_sheet_header[0]]
            ...
            if file_count:
                ...
                file_count_data[self.filewise_count_header[1]].extend(
                    [driver_name] * len(filenames)
                )
                file_count_data[self.filewise_count_header[2]].extend(filecounts)

        self.df_data_dict[self.file_wise_calculation] = [
            pd.DataFrame(file_count_data, index=None)
        ]

    def _generate_driverwise_count_sheet_data(self):
        """
        This method generates the driverwise count from the dataframe contained in the
        filewise calculation sheet
        """
        df_filewise_count = self.df_data_dict[self.file_wise_calculation][0]

        # Grouping by 'driver' and summing 'total count'
        df_driverwise_count = df_filewise_count.groupby(
            self.filewise_count_header[1], as_index=False
        ).agg({self.filewise_count_header[2]: "sum"})

        # Removing index column
        df_driverwise_count = df_driverwise_count.reset_index(drop=True)

        # Adding zero count drivers / cfgstat
        drivers = [driver.lower() for driver in self.df_data_dict]
        for driver in self.df_data_dict:
            driver = driver.lower()
            if (
                driver != self.file_wise_calculation.casefold()
                and driver
                not in self.df_data_dict[self.file_wise_calculation][0][
                    self.filewise_count_header[
                        1
                    ]  # refers to filewise position for driver header
                ].tolist()
            ):
                df_driverwise_count.loc[len(df_driverwise_count.index)] = [driver, 0]

        # Convert 'Driver' column to categorical with specified order
        df_driverwise_count[self.filewise_count_header[1]] = pd.Categorical(
            df_driverwise_count[self.filewise_count_header[1]],
            categories=drivers,
            ordered=True,
        )

        # Sort the DataFrame based on the categorical 'Driver' column
        df_driverwise_count = df_driverwise_count.sort_values(
            self.filewise_count_header[1]
    ...
    def process_log_file(
        self,
        is_driver: bool,
        cfgstat_filter_pattern: Any,
        driver: str,
        driver_filter_pattern: Any,
        log_file: str,
    ) -> dict:
        """
        ...
        with open(log_file_path, mode="r+", encoding="utf-8") as l_file:
            ...
            for line in file_data:
                ...
                sheet_filter_pattern = (
                    driver_filter_pattern if is_driver else cfgstat_filter_pattern
                )
                if (
                    match
                    ...
                    df_file_data = pd.concat([df_file_data, df_row], ignore_index=True)
                    if (
                        match
                        and re.match(sheet_filter_pattern, match.group(1))
                        and block_open is False
            ...
            for index, row in df_file_data.iterrows():
                if (
                    pd.isnull(row[self.warnings_sheet_header[3]])
                    or row[self.warnings_sheet_header[3]] == ""
                ):
                    if (
                        0 <= index < len(df_file_data) - 1
                        and row[self.warnings_sheet_header[0]]
                        == df_file_data.at[index + 1, self.warnings_sheet_header[0]]
                    ):
                        row[self.warnings_sheet_header[3]] = df_file_data.at[
                            index + 1, self.warnings_sheet_header[3]
                        ]
                        df_file_data.at[
                            index + 1, self.warnings_sheet_header[3]
                        ] = ""  # Clear the merged row

            # drop rows with no Warning
            blank_rows = df_file_data[
                (df_file_data[self.warnings_sheet_header[2]].str.strip() == "")
            ]
            df_file_data = df_file_data.drop(blank_rows.index)
            # clean data in Warnings and Note stop with line ending with ";"
            df_file_data[self.warnings_sheet_header[2]] = df_file_data[
                self.warnings_sheet_header[2]
            ].apply(lambda x: str(x).split(";", 1)[0] + ";" if str(x).strip() else x)
            df_file_data[self.warnings_sheet_header[3]] = df_file_data[
                self.warnings_sheet_header[3]
            ].apply(lambda x: str(x).split(";", 1)[0] + ";" if str(x).strip() else x)
            ...
            if is_driver:
                shared_dict[driver] = (
                    shared_dict[driver].append(df_file_data)
                    if driver in shared_dict
                    else [df_file_data]
                )
            else:
                if self.cfgstat_name in shared_dict:
                    shared_dict[self.cfgstat_name].append(
                        df_file_data
                    )  # pragma: no cover - verified that it is covered integration
                else:
